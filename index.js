...
// Periodically reset nicknames if users manually edit them
setInterval(async () => {
  for (const draft of drafts.values()) {
    for (const p of [...draft.team1, ...draft.team2]) {
      if (!p.stats) p.stats = { wins: 0, losses: 0, streak: 0 };
      if (!p.nickname?.includes(`🏆${p.stats.wins}`)) {
        await setNick(p, p.stats);
      }
    }
  }
}, 10000);
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    if (interaction.customId === "random_teams") {
      ...
      const originalVC = {};
      for (const p of players) originalVC[p.id] = p.voice.channelId;
    ...
    if (["end", "t1", "t2"].includes(interaction.customId)) {
      ...
      for (const p of [...draft.team1, ...draft.team2]) {
        await p.voice.setChannel(draft.originalVC[p.id]).catch(() => {});
      }
